...
| D41 | Where email retries live (2026-07-22) | **Outbox-driven, not broker-driven** | The `outbox_jobs` row is the source of truth: it carries `attempts` and `next_attempt_at`, the relay re-publishes jobs due for retry with backoff (1m → 5m → 30m → 2h), and exhaustion marks the row `DEAD`. RabbitMQ is pure transport. All retry state stays queryable in SQL and survives a broker restart; no dead-letter-exchange plumbing or plugins are needed. |
| D42 | Email provider sequencing (2026-07-22) | **Logging adapter first, Gmail behind the same port** | The pipeline is built and tested against a capturing `EmailSender`, so every message is assertable without credentials. The Gmail SMTP adapter is one isolated class chosen by config; an app password is only needed for the live send. |
| D43 | Pre-event reminders (2026-07-22) | **In this phase, one per holder per event** | A scheduled job finds events ~24 h out and enqueues a reminder through the same outbox pipeline, keyed `REMINDER:{eventId}:{userId}` so each holder is reminded exactly once. Completes FR-18's email list. |
| D44 | Raw tokens in email jobs (2026-07-22) | **Payload holds the raw token until sent** | Verification and reset links need the raw single-use token, but the auth store keeps only its hash. The raw value therefore lives in the outbox row's payload until the email is sent and the row is cleared — a brief, transient exposure of a short-lived token, distinct from the long-lived hash-only store the tokens' security rests on. |
